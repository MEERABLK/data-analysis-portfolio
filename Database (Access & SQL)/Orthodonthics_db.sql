...
INSERT INTO Appointments (AppointmentID, AppointmentType, DateTime, ClientID, DentistID) VALUES
(306, 'Braces Adjustment', '2024-10-28 11:00:00', 105, 204);--to add for two apointments on the same day
...
1.	Create a procedure that selects all clients who have booked an appointment with a dentist with a specific specialization (using EXISTS)
...
2.	Create a function that gets and shows all clients whose emails end with ‘@gmail.com’.
...
3.	Create a sequence beginning with a value of 901 and that increments by 1. Then begin a transaction that updates all the PaymentIDs of the Payment table with the sequence. If any of them equal 904, then the transaction will be rollbacked; else, transaction commits.
CREATE SEQUENCE seq
START WITH 901
INCREMENT BY 1
NO cycle;
...
4.	 Create an index for performance on the ClientID foreign key in the Appointments table
CREATE INDEX idx_ClientID ON Appointments (ClientID);
 
5.	Create a view that shows client details with their appointment information.
...
6.	Create a trigger that prevents creation, deletion, and alteration of table.
...
7.	Retrieve the number of appointments made per month
SELECT MONTH(DateTime) AS Month, COUNT(*) AS NumAppointments
FROM Appointments
GROUP BY MONTH(DateTime);

8.	 Create a procedure that calculates the total bill amount for given client:
